...
const SECTORS = {
  zone_black_sea: {
    ...
    meetingCoords: [44.61, 33.53],
    objectPhoto: "https://picsum.photos/800/500?random=1",
    mapPhoto: "https://picsum.photos/800/500?random=2",
    whatHappened:
      "Аварийный сброс нефтепродуктов с проходящего судна в штормовых условиях (3 дня назад).",
    ecoDamage:
      "Высокое загрязнение прибрежной полосы и акватории (класс опасности II).",
    economicDamage:
      "Убытки регионального туризма и рыболовного промысла оцениваются в 14.2 млн руб.",
    faunaImpact:
      "Под угрозой популяция прибрежных птиц и черноморских дельфинов (риск гибели из-за интоксикации).",
    isOngoing:
      "Да, фиксируется локальное поступление мазута с придонных течений. Открыт экстренный набор волонтеров (выезд 15 сентября).",
    timeWithHuman:
      "2–3 недели (при участии волонтерских отрядов и спецтехники).",
  },
  zone_baltic: {
    ...
    meetingCoords: [54.64, 19.98],
    objectPhoto: "https://picsum.photos/800/500?random=3",
    mapPhoto: "https://picsum.photos/800/500?random=4",
    whatHappened:
      "Нанос бытовых и промышленных пластиковых отходов штормовыми течениями с судоходных путей.",
    ecoDamage:
      "Накопление микропластика в прибрежном грунте, разрушение естественного ландшафта.",
    economicDamage:
      "Снижение рекреационной привлекательности пляжной зоны, затраты муниципалитета на вывоз мусора.",
    faunaImpact:
      "Риск заглатывания пластиковых элементов чайками, рыбами и местными млекопитающими.",
    isOngoing:
      "Периодическое поступление. Запущено планирование генеральной уборки силами волонтерского десанта.",
  ...
  zone_arctic: {
    ...
    meetingCoords: [69.025, 33.08],
    objectPhoto: "https://picsum.photos/800/500?random=5",
    mapPhoto: "https://picsum.photos/800/500?random=6",
    whatHappened:
      "Обнаружение герметичного, но подвергшегося сильной коррозии промышленного спецконтейнера прошлых лет.",
    ecoDamage:
      "Локальное радиационное и токсическое заражение прибрежного грунта.",
    economicDamage:
      "Угроза остановки северных портовых проектов и традиционного рыбного промысла.",
    faunaImpact:
      "Прямая угроза арктическим экосистемам, птичьим базарам и лежбищам ластоногих.",
    isOngoing:
      "Контейнер зафиксирован стационарно со спутника. Волонтеров не допускают, идет подготовка к эвакуации силами специализированного МЧС-отряда.",
...
Object.keys(SECTORS).forEach((zoneKey) => {
  ...
  bot.action(`map_photo_${zoneKey}`, async (ctx) => {
    ...
    try {
      ...
      ctx.reply(
        `🗺 <b>Схема ДЗЗ и расположение:</b> ${zone.name}\nКоординаты: <code>${zone.coords.join(", ")}</code>`,
        { parse_mode: "HTML" },
      );
    }
  });
  ...
  bot.action(`apply_${zoneKey}`, async (ctx) => {
    ...
    const userId = ctx.from.id;
    const userName =
      `${ctx.from.first_name || "Волонтер"} ${ctx.from.last_name || ""}`.trim();

    const existingIndex = volunteerApplications.findIndex(
      (a) => a.userId === userId && a.zoneKey === zoneKey,
    );
    if (existingIndex !== -1) {
...
bot.action("my_status", async (ctx) => {
  ...
  if (userApps.length === 0) {
    return ctx.reply(
      "📌 У вас пока нет активных заявок на уборку. Выберите сектор через меню «Выбрать зону уборки».",
    );
  }
...
bot.action("start_report", async (ctx) => {
  await safeAnswer(ctx);
  ctx.reply(
    "📷 Пожалуйста, отправьте в чат <b>фотографию убранного участка</b>, и система передаст ее инспектору ООПТ.",
    { parse_mode: "HTML" },
  );
});
...
bot.action("admin_requests", async (ctx) => {
  await safeAnswer(ctx);
  const pendingApps = volunteerApplications.filter(
  ...
  const buttons = pendingApps.map((app) => [
    Markup.button.callback(
      `✅ Подтвердить: ${app.name} (${app.zoneName})`,
      `approve_${app.userId}_${app.zoneKey}`,
    ),
  ]);
...
bot.action(/^approve_(\d+)_(.+)$/, async (ctx) => {
  ...
  const app = volunteerApplications.find(
    (a) => a.userId === targetUserId && a.zoneKey === zoneKey,
  );
  if (app) {
    app.status = "approved";
    try {
  ...
  const buttons = pendingApps.map((a) => [
    Markup.button.callback(
      `✅ Подтвердить: ${a.name} (${a.zoneName})`,
      `approve_${a.userId}_${a.zoneKey}`,
    ),
  ]);
  buttons.push([Markup.button.callback("🔙 Назад в панель", "admin_home")]);
...
bot.action("admin_reports", async (ctx) => {
  await safeAnswer(ctx);
  ctx.reply(
    "📷 <b>Поступил новый фотоотчет волонтера:</b>\nСектор: Черноморский сектор-А",
    { parse_mode: "HTML" },
  );
  try {
    await ctx.replyWithPhoto("https://picsum.photos/800/500?report=1", {
      caption:
        "Утвердить ликвидацию загрязнения и перевести сектор в статус «Очищено»?",
      ...Markup.inlineKeyboard([
        [
          Markup.button.callback(
            "🟢 Утвердить и закрыть сектор",
            "close_sector",
          ),
        ],
      ]),
...
bot.action("close_sector", async (ctx) => {
  ...
  ctx
    .editMessageCaption(
      "✅ Сектор официально переведен в статус <b>«Ликвидировано»</b> на платформе!",
      { parse_mode: "HTML" },
    )
    .catch(() => {});
});
...
bot.on("text", (ctx) => {
  ...
  if (waitingForAdminPassword.has(userId)) {
    ...
    ctx.reply(
      "Используйте инлайн-кнопки меню или команду /start для перезапуска.",
    );
  }
});
